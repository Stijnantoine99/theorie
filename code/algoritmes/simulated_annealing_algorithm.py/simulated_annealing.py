...
class Move(): 
    ...
    def move_maison(self, coordinaten_maison, coordinaten_bungalow, coordinaten_single):
        ...
        # set counter for indexation within coordinates lists and the default annealing temperature
        counter = 0 
        temperature = 8
        ...
        for coordinates in coordinaten_maison:
            ...
            # save the starting coordinates of the maison
            ...
            if self.possible_move(x, y, 12, 10, 6) == True:
                ...
                coordinaten_maison[counter] = top_left
            ...
            # start simulated annealing if house has not moved
            if begin_coordinates == best_coor:
                coor_list_annealing = []
                yield_house_annealing = []
                acceptance_probabilties = []
                
                # determine size step for house if temperature is higher than 4
                if temperature > 4:
                    step = random.randrange(temperature - 4, temperature)

                # determine size step for house if T is in range between 2 and 4
                elif temperature > 1 and temperature < 5:
                    step = random.randrange(1, temperature)

                else:
                    step = 1

                # top left coordinates
                x = (coordinates[0] - step)
                y = (coordinates[1] - step)
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save top left coordinates
                    top_left = [x,y]
                    coor_list_annealing.append(top_left)
                    coordinaten_maison[counter] = top_left
                    
                    # save top left price
                    new = price.total(coordinaten_maison, coordinaten_bungalow, coordinaten_single)
                    yield_house_annealing.append(new)

                    # remove moved house from the gridmap, and place water when obligatory free space of house is in water
                    self.neighbourhood[y-6:y+16, x-6:x+18] = 0
                    self.place.place_water()

                    # calculate acceptance probability and add to list
                    acceptance_new = new/1000000
                    acceptance_begin = begin_price/1000000
                    acceptance = round(2 ** (acceptance_begin - acceptance_new)/temperature, 3)
                    acceptance_probabilties.append(acceptance)

                # add zeroes to coordinates and prices list to ensure the length of the lists remain equal
                else:
                    coor_list_annealing.append([0,0])
                    yield_house_annealing.append(0.00)
                    acceptance_probabilties.append(1.00)
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save top coordinates
                    top = [x,y]
                    coor_list_annealing.append(top)
                    coordinaten_maison[counter] = top
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save top right coordinates
                    top_right = [x,y]
                    coor_list_annealing.append(top_right)
                    coordinaten_maison[counter] = top_right
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save right coordinates
                    right = [x,y]
                    coor_list_annealing.append(right)
                    coordinaten_maison[counter] = right
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save bottom right coordinates
                    bottom_right = [x,y]
                    coor_list_annealing.append(bottom_right)
                    coordinaten_maison[counter] = bottom_right
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save bottom coordinates
                    bottom = [x,y]
                    coor_list_annealing.append(bottom)
                    coordinaten_maison[counter] = bottom
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save bottom left coordinates
                    bottom_left = [x,y]
                    coor_list_annealing.append(bottom_left)
                    coordinaten_maison[counter] = bottom_left
                ...
                if self.possible_move(x, y, 12, 10, 6) == True:
                    ...
                    # save left coordinates
                    left = [x,y]
                    coor_list_annealing.append(left)
                    coordinaten_maison[counter] = left
                ...
                # select move from annealing steps
                under_accep_prob = []
                for step_probability in acceptance_probabilties:

                    # if the annealing step is lower than accep. probability add to list
                    if step_probability < acceptance_probability:
                        under_accep_prob.append(step_probability)

                # get random step from possible annealing steps list
                try:
                    get_step = random.choice(under_accep_prob)

                    # determine the coordinates from the annealing step
                    index = acceptance_probabilties.index(get_step)
                    best_coor = coor_list_annealing[index]

                    y = best_coor[1]
                    x = best_coor[0]
                
                # if no annealing steps are possible continue
                except IndexError:
                    print("No annealing possible")
    ...
    def move_single(self, coordinaten_maison, coordinaten_bungalow, coordinaten_single):
        ...
        for coordinates in coordinaten_maison:
            ...
            if self.possible_move(x,y,8,8,2) == True:
                ...
                top_left = [x,y]
                list.append(top_left)
                coordinaten_single[counter] = top_left
    ...
    def move_bungalow(self, coordinaten_maison, coordinaten_bungalow, coordinaten_single):
        ...
        for coordinates in coordinaten_maison:
            ...
            if self.possible_move(x, y, 11, 7, 3) == True:
                ...
                top_left = [x,y]
                list.append(top_left)
                coordinaten_bungalow[counter] = top_left
